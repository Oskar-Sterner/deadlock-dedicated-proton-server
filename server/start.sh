...
# Sync only files that actually differ — a blind cp -rf forces every
# lowerdir file to copy up to the overlayfs upperdir when the game dir is
# an overlay mount, ballooning per-server disk usage by ~37 GB.
